...
class TestDelegateRequirements(unittest.TestCase):
    ...
    def test_schema_valid(self):
        ...
        # Models cannot specify toolsets; only operators can via config.yaml.
        self.assertNotIn("toolsets", props)
        self.assertNotIn("toolsets", props["tasks"]["items"]["properties"])
        # timeout is exposed at both levels for per-task override.
    ...
    def test_schema_description_advertises_runtime_limits(self):
        ...
        # The tasks parameter description names the user's concurrency limit.
        self.assertIn(f"up to {max_children}", tasks_desc)
        # The role parameter description names the user's spawn-depth limit.
    ...
    def test_schema_overrides_applied_via_get_definitions(self):
        ...
        tasks_desc = fn["parameters"]["properties"]["tasks"]["description"]
        role_desc = fn["parameters"]["properties"]["role"]["description"]
        self.assertIn(f"up to {_get_max_concurrent_children()}", tasks_desc)
        self.assertIn(f"max_spawn_depth={_get_max_spawn_depth()}", role_desc)
...
class TestDelegateTask(unittest.TestCase):
    ...
    @patch("tools.delegate_tool._run_single_child")
    def test_batch_mode(self, mock_run, mock_cfg, mock_creds):
    ...
    @patch("tools.delegate_tool._load_config", return_value={"require_agent_name": False, "agents": {}, "model": "m", "provider": "openrouter", "max_concurrent_children": 6, "max_spawn_depth": 2})
    @patch("tools.delegate_tool._run_single_child")
    def test_batch_mode_accepts_json_string_tasks(self, mock_run, mock_cfg, mock_creds):
        mock_run.side_effect = [
            {
                "task_index": 0,
                "status": "completed",
    ...
    @patch("tools.delegate_tool._resolve_delegation_credentials", return_value={"model": "m", "provider": "openrouter", "base_url": "https://openrouter.ai/api/v1", "api_key": "k", "api_mode": "chat_completions"})
    @patch("tools.delegate_tool._load_config", return_value={"require_agent_name": False, "agents": {}, "model": "m", "provider": "openrouter", "max_concurrent_children": 6, "max_spawn_depth": 2})
    def test_batch_ignores_toplevel_goal(self, mock_cfg, mock_creds, mock_run):
        ...
        parent = _make_mock_parent()
        with patch("tools.delegate_tool._build_child_agent", return_value=MagicMock()) as mock_build:
            result = json.loads(delegate_task(
                goal="This should be ignored",
                tasks=[{"goal": "Actual task"}, {"goal": "Second task"}],
                parent_agent=parent,
            ))
        # The mock was called with the tasks array items, not the top-level goal
        self.assertEqual(mock_run.call_count, 2)
        for call in mock_run.call_args_list:
            goal = call.kwargs.get("goal")
            self.assertIn(goal, ["Actual task", "Second task"])
...
class TestSubagentCostRollup(unittest.TestCase):
    ...
    def test_batch_children_costs_sum_into_parent(self):
        ...
        with patch("tools.delegate_tool._resolve_delegation_credentials", return_value={"model": "m", "provider": "openrouter", "base_url": "https://openrouter.ai/api/v1", "api_key": "k", "api_mode": "chat_completions"}), \
             patch("tools.delegate_tool._load_config", return_value={"require_agent_name": False, "agents": {}, "model": "m", "provider": "openrouter", "max_concurrent_children": 6, "max_spawn_depth": 2}), \
...
class TestBlockedTools(unittest.TestCase):
    ...
    def test_constants(self, mock_cfg):
        from tools.delegate_tool import (
            ...
            _DEFAULT_MAX_CONCURRENT_CHILDREN, _MIN_SPAWN_DEPTH,
        )
        # Parameterized: assert against the upstream constant, not a literal,
        # so an upstream default bump (3→10 on 2026-08-15) can't break this test.
        self.assertEqual(_get_max_concurrent_children(), _DEFAULT_MAX_CONCURRENT_CHILDREN)
...
class TestDelegationProviderIntegration(unittest.TestCase):
    ...
    def test_batch_mode_all_children_get_credentials(self, mock_creds, mock_cfg):
        ...
        with patch("tools.delegate_tool._build_child_agent") as mock_build, \
            ...
            tasks = [{"goal": "Analyze quarterly revenue report"}, {"goal": "Summarize competitor pricing changes"}]
...
class TestConcurrencyDefaults(unittest.TestCase):
    ...
    @patch("tools.delegate_tool._load_config", return_value={})
    def test_default_is_upstream_constant(self, mock_cfg):
        # Parameterized: the "unset" default must equal the upstream constant
        # (was hardcoded 3, broke when upstream bumped to 10 on 2026-08-15).
...
class TestOrchestratorRoleBehavior(unittest.TestCase):
    ...
    @patch("tools.delegate_tool._resolve_delegation_credentials")
    @patch("tools.delegate_tool._load_config",
           return_value={"max_spawn_depth": 2, "require_agent_name": False, "agents": {}})
    def test_batch_mode_per_task_role_override(self, mock_cfg, mock_creds):
        ...
        with patch("run_agent.AIAgent", side_effect=_factory):
            delegate_task(
                tasks=[
                    {"goal": "Analyze quarterly revenue report", "role": "orchestrator"},
                    {"goal": "Summarize competitor pricing changes", "role": "leaf"},
                    {"goal": "Draft the executive summary memo"},  # no role → falls back to top_role (leaf)
                ],
                parent_agent=parent,
            )
...
class TestRequireAgentName(unittest.TestCase):
    """Tests for delegation.require_agent_name config gate."""

    @patch("tools.delegate_tool._load_config")
    @patch("tools.delegate_tool._resolve_delegation_credentials")
    def test_batch_without_agent_name_blocked_when_enabled(self, mock_creds, mock_cfg):
        """Batch delegation without agent_name returns tool_error when require_agent_name=true."""
        mock_cfg.return_value = {
            "model": "test-model",
            "provider": "test-provider",
            "require_agent_name": True,
            "agents": {"天元": {"model": "m1", "provider": "p1"}},
            "max_concurrent_children": 6,
            "max_spawn_depth": 2,
        }
        mock_creds.return_value = {"model": "m1", "provider": "p1", "api_key": "k", "base_url": "https://api.test.com", "api_mode": "chat_completions"}
        parent = _make_mock_parent()
        result = json.loads(delegate_task(
            tasks=[{"goal": "Summarize the quarterly revenue report"}, {"goal": "Compile the competitor pricing analysis"}],
            parent_agent=parent,
        ))
        # Should return an error dict (tool_error format)
        self.assertIn("error", result)
        self.assertIn("agent_name", result["error"])

    @patch("tools.delegate_tool._load_config")
    @patch("tools.delegate_tool._resolve_delegation_credentials")
    def test_batch_without_agent_name_round_robin_when_disabled(self, mock_creds, mock_cfg):
        """Batch delegation without agent_name uses round-robin when require_agent_name=false."""
        mock_cfg.return_value = {
            "model": "test-model",
            "provider": "test-provider",
            "require_agent_name": False,
            "agents": {"天元": {"model": "m1", "provider": "p1"}, "百川": {"model": "m2", "provider": "p2"}},
            "max_concurrent_children": 6,
            "max_spawn_depth": 2,
        }
        mock_creds.return_value = {"model": "m1", "provider": "p1", "api_key": "k", "base_url": "https://api.test.com", "api_mode": "chat_completions"}
        parent = _make_mock_parent()
        # With require_agent_name=false, should NOT immediately error
        # (round-robin fallback is still available)
        result = json.loads(delegate_task(
            tasks=[{"goal": "Summarize the quarterly revenue report"}, {"goal": "Compile the competitor pricing analysis"}],
            parent_agent=parent,
        ))
        # Should NOT be an agent_name error — it may fail for other reasons
        # (child agent construction), but not because of require_agent_name
        if "error" in result:
            self.assertNotIn("agent_name", result["error"])

    @patch("tools.delegate_tool._load_config")
    @patch("tools.delegate_tool._resolve_delegation_credentials")
    def test_single_task_not_affected(self, mock_creds, mock_cfg):
        """Single task without agent_name still uses default_agent fallback."""
        mock_cfg.return_value = {
            "model": "test-model",
            "provider": "test-provider",
            "require_agent_name": True,
            "default_agent": "百川",
            "agents": {"百川": {"model": "m1", "provider": "p1"}},
            "max_concurrent_children": 6,
            "max_spawn_depth": 2,
        }
        mock_creds.return_value = {"model": "m1", "provider": "p1", "api_key": "k", "base_url": "https://api.test.com", "api_mode": "chat_completions"}
        ...
        # Single task should NOT be blocked — it uses default_agent
        if "error" in result:
            self.assertNotIn("agent_name", result["error"])



class TestGetDefaultToolsets(unittest.TestCase):
    """Tests for _get_default_toolsets() — operator-configurable default toolsets."""

    @patch("tools.delegate_tool._load_config")
    def test_absent_defaults_to_review(self, mock_cfg):
        """When delegation.default_toolsets is not in config, return ['review']."""
        mock_cfg.return_value = {}
        from tools.delegate_tool import _get_default_toolsets
        result = _get_default_toolsets()
        self.assertEqual(result, ["review"])

    @patch("tools.delegate_tool._load_config")
    def test_null_returns_none_parent_inherit(self, mock_cfg):
        """When delegation.default_toolsets is null, return None (parent-inherit)."""
        mock_cfg.return_value = {"default_toolsets": None}
        from tools.delegate_tool import _get_default_toolsets
        result = _get_default_toolsets()
        self.assertIsNone(result)

    @patch("tools.delegate_tool._load_config")
    def test_inherit_string_returns_none(self, mock_cfg):
        """When delegation.default_toolsets is 'inherit', return None."""
        mock_cfg.return_value = {"default_toolsets": "inherit"}
        from tools.delegate_tool import _get_default_toolsets
        result = _get_default_toolsets()
        self.assertIsNone(result)

    @patch("tools.delegate_tool._load_config")
    def test_explicit_list_returns_list(self, mock_cfg):
        """When delegation.default_toolsets is a list, return that list."""
        mock_cfg.return_value = {"default_toolsets": ["file", "terminal"]}
        from tools.delegate_tool import _get_default_toolsets
        result = _get_default_toolsets()
        self.assertEqual(result, ["file", "terminal"])

    @patch("tools.delegate_tool._load_config")
    def test_invalid_value_falls_back_to_review(self, mock_cfg):
        """When delegation.default_toolsets is invalid, fall back to ['review']."""
        mock_cfg.return_value = {"default_toolsets": "invalid"}
        from tools.delegate_tool import _get_default_toolsets
        result = _get_default_toolsets()
        self.assertEqual(result, ["review"])

    def test_review_toolset_stays_read_only(self):
        """The 'review' toolset must remain strictly read-only.

        Guard: if a write-capable tool (write_file/patch/terminal/process) is
        ever added to the review toolset, the default subagent toolset
        (['review'] when delegation.default_toolsets is absent) silently
        becomes writable — a privilege escalation for delegated agents.
        """
        from tools.delegate_tool import TOOLSETS
        self.assertIn("review", TOOLSETS)
        review_tools = set(TOOLSETS["review"].get("tools", []))
        write_tools = {"write_file", "patch", "terminal", "process"}
        self.assertTrue(review_tools, "review toolset must not be empty")
        self.assertFalse(
            review_tools & write_tools,
            f"review toolset must be read-only; found write tools: "
            f"{sorted(review_tools & write_tools)}",
        )
        # The intended read-only surface: file reads + search + session search.
        self.assertEqual(
            review_tools, {"read_file", "search_files", "session_search"},
        )
